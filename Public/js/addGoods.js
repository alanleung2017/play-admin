...
$(function(){
    ...
    var FileInput = function () {
        ...
        oFile.Init = function(ctrlName, uploadUrl) {
            ...
            //导入文件上传完成之后的事件
            $("#goods_photo_url").on("fileuploaded", function (event, data, previewId, index) {
                //alert(JSON.stringify(data.response));
                var myResponse = data.response;
                //alert(myResponse.content);
                if(myResponse.status == 1){
                    $('#goodsForm').append('<input type="hidden" name="goods_photo_id" value="'+myResponse.content+'"/>');
                }else{
                    alert(myResponse.content);
                }
            });
    ...
    oFileInput.Init("goods_photo_url", ThinkPHP['MODULE'] + "/Goods/uploadGoodsPic");

    $("#theme_shop").click(function(){
    ...
    $("#theme").click(function(){
        ...
        $.ajax({
            type : 'POST',
            url : ThinkPHP['MODULE'] + '/Goods/getShop',
            dataType : 'json',
            data : {
                theme_id : $('select[name=theme_id]').val(),
            },
            success : function (response,status, xhr) {
                var str = '';
                $.each(response, function(i,val){
                    $.each(val, function(i,vall) {
                        var data = vall;
                        if(i == 'shop_id'){
                            str += "<option value='"+data+"'>";
                        }else if(i == 'shop_name'){
                            str += data+"</option>";
                        }
                    });
...
////                        alert(data.msg);
//                    if(data.msg == true){
//                        $('#shopErrMsg').attr("onSubmit","return false");
//                        $('#shopErrMsg').text('商店已存在！');
//                    } else {
//                        $('#shopErrMsg').attr("onSubmit","return true");
//                        $('#shopErrMsg').text('');
//                    }
//                },
//                error: function(data) {
